...
class ParseMaster:
    ...
    def add(self, expression="^$", replacement=None):
        ...
        length = len(ParseMaster.GROUPS.findall(
                    self._internalEscape(str(expression)))) + 1
        # does the pattern deal with sub-expressions?
        if (isinstance(replacement, str) and
            ParseMaster.SUB_REPLACE.match(replacement)):
...
class JavaScriptPacker:
    ...
    def __init__(self):
        self._basicCompressionParseMaster = \
            self.getCompressionParseMaster(False)
        self._specialCompressionParseMaster = \
            self.getCompressionParseMaster(True)
        ...
        if all:
            ...
            for word in all:
                ...
                if word not in count:
                    ...
                    # make a dictionary of all of the protected words
    ...
    def encodeSpecialChars(self, script):
        ...
        def repl(match, offset):
            ...
            return match.group(offset + 1)[start:start+length] + \
    ...
    def bootStrap(self, packed, keywords, encoding, fastDecode):
        ...
        #packed = [packed[x*10000:(x+1)*10000]
        #            for x in range((len(packed)/10000)+1)]
        #packed = "'" + "'+\n'".join(packed) + "'\n"
        packed = "'" + self.escape(packed) + "'"
